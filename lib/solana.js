 ...
 */
const establishConnection = async (url = 'https://devnet.solana.com') => {
...
const createPayer = async (secretKey) => {
  ...
  let payer = secretKey ? new Account(Buffer.from(secretKey, 'hex')) : new Account();
  jump: try {
    if (secretKey) {
      logger.log('The current payer is overriden. Details:');
      break jump;
    } else {
